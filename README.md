...
`rank.py` computes a **global percentile-based ranking** of all your Non-Human Identities based on their control-plane and data-plane scores (WAR norm and blast radius), grouped by fibers. NHI Rank provides a compact, high-information view of identity risk across very large Azure environments.

### What NHI Rank does
- Reads CSV generated by `silhouette.py`
- Normalizes WAR (control plane permissions) and blast radius (data plane permissions) 
- Computes a combined RBAC permissions score per fiber
- Assigns each NHI to a **percentile bucket**
- Produces a CSV showing:
  - fiber id  
  - name of a representative NHI in fiber
  - WAR  
  - blast radius  
  - fiber rank  
  - percentile group  
...
After running silhouette in --frs mode (see below to understand this mode), generate fibers then run NHI_rank:

```
python3 gen_fibers.py --asof YYYY-MM-DD
python3 NHI_rank/rank.py --asof YYYY-MM-DD 
```

It will print a fully ranked view of your entire NHI landscape on stdout.

For an interactive browser, run the tool with the --i option:

```
python3 NHI_rank/rank.py --asof YYYY-MM-DD --interactive
```
