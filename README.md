...
Silhouette now includes **NHI Rank**, located in the `NHI_rank/` subdirectory.

## Overview

NHI Rank is an interactive visualization tool that displays fiber and NHI data through a hierarchical drill-down interface. It uses a terminal-based curses UI to show statistical distributions and detailed listings of fibers and their constituent NHIs.
...
First, run silhouette in --frs mode (see below to understand this mode), then generate today's fibers or the fibers of a previous date (YYYY-MM-DD):

```
python3 silhouette --frs --live
python3 gen_fibers.py --asof YYYY-MM-DD
```

If you didnt run silhouette on YYYY-MM-DD, then gen_fibers.py will not work on this date.

You are now ready to launch NHI rank.

### Command-line Options

```bash
# With real data from a specific date
python3 rank.py --asof YYYY-MM-DD

# With real data in interactive mode
python3 rank.py --asof YYYY-MM-DD --interactive

# With demo data (no date required)
python3 rank.py --demo

# With demo data in interactive mode
python3 rank.py --demo --interactive
```

### Input Files (non-demo mode)

- `sorted_NHIs_YYYY-MM-DD.csv` - NHI data with risk metrics
- `sorted_fibers_YYYY-MM-DD.csv` - Fiber groupings with blast radius

### Output (non-interactive mode)

Displays a sorted table of fibers with:
- Fiber ID
- Representative NHI name
- κ (kappa equivalent)
- WAR (Weighted Attack Risk)
- Combined risk score
- Population (number of NHIs in fiber)
- Percentile ranking

## Interactive Mode Navigation

The interactive mode provides a **4-level drill-down interface** for exploring fiber data:

### Screen 1: Decile Bar Chart
**Purpose:** Overview of fiber distribution across risk deciles (0-9)

**Display:**
- Bar chart showing fiber count in each decile
- Decile 0 = lowest 10%, Decile 9 = highest 10% by combined risk

**Controls:**
- `0-9` - Drill down into specific decile
- `Q/q` - Quit application

---

### Screen 2: Percentile Histogram
**Purpose:** Fine-grained distribution within a selected decile

**Display:**
- Histogram of fiber counts across 10 percentile bins within the decile
- Bins labeled as `XY` where X = decile, Y = percentile bin (e.g., "05" = decile 0, bin 5)

**Controls:**
- `0-9` - Drill down into specific percentile bin
- `ESC` - Return to Screen 1 (Decile Bar Chart)
- `Q/q` - Quit application

---

### Screen 3: Fiber List
**Purpose:** Detailed list of fibers in a specific percentile range

**Display:**
- Table with columns: `#`, `FIBER_ID`, `WAR`, `KE` (kappa equiv), `PERCENTILE`, `POP`
- **Selected fiber highlighted in reverse video** (default: top fiber)
- Scroll indicator shows position when list exceeds screen height

**Controls:**
- `↑/↓` - Move selection up/down (auto-scrolls when needed)
- `SPACE` - View NHI details for selected fiber (→ Screen 4)
- `ESC` - Return to Screen 2 (Percentile Histogram)
- `Q/q` - Quit application

**Notes:**
- The selected fiber is always clearly marked with reverse video highlighting
- Selection automatically scrolls the view to keep focused fiber visible

---

### Screen 4: NHI Detail View
**Purpose:** Show all NHIs belonging to the selected fiber

**Display:**
- Table with columns: `#`, `NHI NAME`, `WAR`, `KE`, `COMBINED`
- Total NHI count displayed at bottom
- Scroll indicator for long lists

**Controls:**
- `↑/↓` - Scroll through NHI list
- `ESC` - Return to Screen 3 (Fiber List)
- `Q/q` - Quit application
...
┌─────────────────────────────────────────────────────────────┐
│ Screen 1: Decile Bar Chart                                  │
│   Press 0-9 → Drill down to decile                          │
│   Press Q   → Exit                                           │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ↓
┌─────────────────────────────────────────────────────────────┐
│ Screen 2: Percentile Histogram (within decile)              │
│   Press 0-9 → Drill down to percentile bin                  │
│   Press ESC → Back to Screen 1                              │
│   Press Q   → Exit                                           │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ↓
┌─────────────────────────────────────────────────────────────┐
│ Screen 3: Fiber List (selectable)                           │
│   Press ↑↓  → Select fiber (reverse video highlight)        │
│   Press SPC → View NHI details                              │
│   Press ESC → Back to Screen 2                              │
│   Press Q   → Exit                                           │
└────────────────┬────────────────────────────────────────────┘
                 │
                 ↓ (Space)
┌─────────────────────────────────────────────────────────────┐
│ Screen 4: NHI Detail View                                   │
│   Press ↑↓  → Scroll through NHIs                           │
│   Press ESC → Back to Screen 3                              │
│   Press Q   → Exit                                           │
└─────────────────────────────────────────────────────────────┘
```
...
### Risk Scoring
- **WAR (Weighted Attack Risk):** Individual NHI risk metric
- **κ (Kappa Equivalent):** Derived from blast radius using depth-based mapping
- **Combined Score:** `(linearized_WAR² + linearized_KE²) + population`
  - Linearization: `1000 × (value - min) / (max - min)` squared
  - Higher combined scores indicate higher-priority fibers

### Fiber Population
- Each fiber can contain multiple NHIs
- The `pop` field shows the count of NHIs in each fiber
- Fiber list shows the **best representative NHI** (highest combined score) from each fiber
- NHI Detail View (Screen 4) shows **all NHIs** in the selected fiber

### Data Processing Pipeline
1. Load NHI and fiber CSVs
2. Map blast radius to kappa using logarithmic depth calculation
3. Linearize and combine risk metrics
4. Merge NHI data with fiber groupings
5. Deduplicate fibers (keeping best representative)
6. Calculate percentile rankings
7. Bin into deciles and percentile ranges

## Exit Behavior

- **Q/q from any screen:** Immediately exits the entire application
- **ESC:** Returns to previous screen (hierarchical navigation)
- Screens without a previous level (Screen 1) do not respond to ESC
