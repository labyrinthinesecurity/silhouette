...
data = {
    ...
    'Count': [diameter_counts.get(i, 0) for i in range(1, 9)]
}
...
def interpolate_median(diameter, cumulative_percentage):
    for i in range(len(cumulative_percentage) - 1):
        if cumulative_percentage[i] < 50 <= cumulative_percentage[i + 1]:
            x0, x1 = diameter[i], diameter[i + 1]
            y0, y1 = cumulative_percentage[i], cumulative_percentage[i + 1]
            return x0 + (x1 - x0) * ((50 - y0) / (y1 - y0))
    return None

median_diameter = interpolate_median(df['Diameter'], df['Cumulative Percentage'])
